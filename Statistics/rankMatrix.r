...
#' @export
rankMatrix.ScreenData <- function(x,
                                  direction=c('low','high'),
                                  group=c('gene', 'sgrna')) {
    direction <- match.arg(direction)
    group <- match.arg(group)
    data <- x$data[, c('gene', 'sgrna', 'score.low', 'score.high')]

    if (group == 'sgrna') {
        data$gene <- unlist(
            apply(data[, c('gene', 'sgrna')], paste, collapse='_')
        )
    } else {}

    if (direction == 'low') {
        ranking <- data[, c('score.low', 'gene')]
    } else {
        ranking <- data[, c('score.high', 'gene')]
    }

    colnames(ranking) <- c('score', 'gene')

    ranking$rank <- rank(
        ranking$score,
        na.last=TRUE,
        ties.method='random'
    ) / length(ranking$score)
    ...
    rmat <- t(unstack(ranking, rank~gene))
